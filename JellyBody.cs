    ...
    private void OnCollisionStay(Collision other) {
        if (other.contacts.Length > 0) {
            ...
            for (int i = 0; i < other.contacts.Length; i++) {
                Vector3 currentContacctpoint = other.contacts[i].point;
                currentContacctpoint += other.contacts[i].normal * mesh.bounds.max.x;
                contactPoints[i] = currentContacctpoint;
            }

            JellyManagement.Instance.AddForceToVerts(contactPoints, rigidBody.mass, gameObject, this);
        }
    }
